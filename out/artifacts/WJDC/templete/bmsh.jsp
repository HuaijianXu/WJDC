    ...
    <table class="table table-hover table-striped table-bordered table-sm">
        ...
        <c:forEach var="l" items="<%=list %>" varStatus="i">
            <tr>
                <td>${l.getJsxqmc()}</td>
                <td>${l.getJgmc() }</td>
                <td id="status${i.index+1 }"><c:if test="${l.getBmsfsh()==-1}"> 未审核 </c:if><c:if test="${l.getBmsfsh()==0}"> 未通过 </c:if><c:if test="${l.getBmsfsh()==1}"> 已通过 </c:if></td>
...
<script>
    ...
    {
        window.location="${pageContext.request.contextPath}/checkDoc?type=notfchecked";
    }
    function showChecked()
    {
        window.location="${pageContext.request.contextPath}/checkDoc?type=fchecked"
    }
    function docheck(index,id)
    {
        ...
        if(status.trim()=="未审核")
        {
            window.location="${pageContext.request.contextPath}/dofCheck?id="+id;
        }
        else
        {
            alert("不具备审核权限或已经审核！");
